...
├── esfera_icosaedro.obj        # Malha OBJ — icosaedro refinado
├── esfera_octaedro.obj         # Malha OBJ — octaedro refinado
├── esfera_tetraedro.obj        # Malha OBJ — tetraedro refinado
└── README.md
```
...
Dada uma função implícita $f(x,y)$ que define uma região no plano ($f \leq 0$ = dentro, $f = 0$ = fronteira, $f > 0$ = fora), queremos classificar subdivisões retangulares do domínio em **dentro**, **fora** ou **fronteira**.

Duas formas testadas para o disco com centro $(0.3, 0.4)$ e raio $0.21$ no quadrado unitário, e para a região $y \geq x^2 + c$ em $[-2, 2]^2$:

### Classificação via TVI

A classificação de cada célula usa o **Teorema do Valor Intermediário** — um resultado que vem de Bolzano (1817), muito antes de ser formalizado por Weierstrass. A ideia é elegante na sua simplicidade: se $f$ é contínua e assume valores positivos e negativos nos cantos de uma célula retangular, então necessariamente $f = 0$ em algum ponto interior. Ou seja, a fronteira cruza a célula.

Na prática, avaliamos $f$ nos 4 cantos:

- Todos $\leq 0$ → célula **dentro**
- Todos $> 0$ → célula **fora**
- Sinais mistos → célula de **fronteira**

É um teste barato (4 avaliações de $f$) e suficiente para as formas desta tarefa, embora possa falhar para curvas que entram e saem da célula sem tocar nenhum canto — nesse caso, seriam necessários oráculos intervalares, como discutido em aula.

### Enumeração uniforme

Divide o domínio numa grade regular $N \times N$. Cada célula é classificada independentemente. É o método mais simples: para $32 \times 32$, temos 1024 células. Funciona, mas gasta a mesma resolução em regiões que são inteiramente dentro ou inteiramente fora — onde não precisamos de detalhe nenhum.
...
<p align="center">
  <img src="disco_1.png" width="85%" alt="Disco: uniforme vs adaptativa"/>
</p>

**Parábola $y \geq x^2$ — Uniforme vs. Adaptativa:**

<p align="center">
  <img src="parabola_c0.png" width="85%" alt="Parábola: uniforme vs adaptativa"/>
</p>

**Evolução do refinamento adaptativo (profundidades 1 a 7):**

<p align="center">
  <img src="refinamento_disco.png" width="90%" alt="Evolução do refinamento"/>
</p>
...
**Variações da parábola:**

<p align="center">
  <img src="parabola_c0.png" width="45%" alt="c=0"/>
  <img src="parabola_c-1.png" width="45%" alt="c=-1"/>
</p>
<p align="center">
  <img src="parabola_c1.png" width="45%" alt="c=1"/>
  <img src="parabola_c0.5.png" width="45%" alt="c=0.5"/>
</p>
<p align="center"><em>De cima para baixo, esquerda para direita: c = 0, −1, 1, 0.5</em></p>
...
Outra questão prática foi a escolha da **profundidade mínima** da Quadtree. Se começo com profundidade mínima 0, a raiz pode ser classificada como "dentro" para discos pequenos no centro do domínio, e o algoritmo nunca refina. Usar profundidade mínima 2 resolve isso, garantindo pelo menos $4^2 = 16$ células antes de começar a decidir se refina ou não.
...
Para cada triângulo $ABC$:

1. Calcular pontos médios: $D = \frac{A+B}{2}$, $E = \frac{B+C}{2}$, $F = \frac{A+C}{2}$
2. Projetar na esfera: $D \leftarrow \frac{D}{\|D\|}$ (e analogamente para $E$, $F$)
3. Substituir por 4 subtriângulos: $(A, D, F)$, $(D, B, E)$, $(F, E, C)$, $(D, E, F)$
...
A cada nível, o número de faces se multiplica por 4. Para o icosaedro: $20 \to 80 \to 320 \to 1280$ faces.
...
A solução é a **tabela hash** sugerida no enunciado: antes de inserir um vértice, arredondo suas coordenadas para 8 casas decimais e uso a tupla $(round(x, 8), round(y, 8), round(z, 8))$ como chave. Se a chave já existe no dicionário, retorno o índice existente. Isso garante zero duplicatas, mesmo com erros de ponto flutuante.
...
<p align="center">
  <img src="evolucao_icosaedro.png" width="90%" alt="Evolução do icosaedro"/>
</p>

**Comparação dos 3 sólidos platônicos (3 níveis de subdivisão):**

<p align="center">
  <img src="comparacao_solidos.png" width="90%" alt="Comparação dos sólidos"/>
</p>

**Esferas geradas (dois ângulos de visualização):**

<p align="center">
  <img src="esfera_tetraedro.png" width="80%" alt="Esfera — tetraedro"/>
</p>
<p align="center"><em>Tetraedro (5 subdivisões) — 2050 vértices, 4096 faces</em></p>

<p align="center">
  <img src="esfera_octaedro.png" width="80%" alt="Esfera — octaedro"/>
</p>
<p align="center"><em>Octaedro (4 subdivisões) — 1026 vértices, 2048 faces</em></p>

<p align="center">
  <img src="esfera_icosaedro.png" width="80%" alt="Esfera — icosaedro"/>
</p>
<p align="center"><em>Icosaedro (3 subdivisões) — 642 vértices, 1280 faces</em></p>

**Métricas de qualidade (3 níveis de refinamento):**

| Sólido | Vértices | Faces | Razão de áreas | Euler (V−E+F) | Erro de área vs. 4π |
|--------|:--------:|:-----:|:--------------:|:-------------:|:--------------------:|
| Tetraedro | 130 | 256 | 6.52 | 2 ✓ | 0.21% |
| Octaedro | 258 | 512 | 2.09 | 2 ✓ | 0.32% |
| Icosaedro | 642 | 1280 | **1.29** | 2 ✓ | 0.48% |

A **razão de áreas** (maior triângulo / menor triângulo) é a métrica mais reveladora: o icosaedro produz triângulos quase iguais (razão 1.29), enquanto o tetraedro tem triângulos muito desiguais (6.52). Todos os vértices estão na esfera unitária com erro de norma da ordem de $10^{-16}$ (precisão de ponto flutuante IEEE 754).
...
- **Tarefa 1**: representação **implícita** — o objeto é definido por uma função $f(x,y)$, e a enumeração espacial classifica o espaço ao redor. O refinamento adaptativo (Quadtree) concentra resolução na fronteira.
...
1. **Bolzano, B.** (1817). *Rein analytischer Beweis des Lehrsatzes, dass zwischen je zwey Werthen, die ein entgegengesetztes Resultat gewähren, wenigstens eine reelle Wurzel der Gleichung liege.* — Prova analítica do TVI, base teórica da classificação de células.

2. **Finkel, R. A. e Bentley, J. L.** (1974). Quad Trees: A Data Structure for Retrieval on Composite Keys. *Acta Informatica*, 4(1), 1–9. — Estrutura de dados Quadtree usada na enumeração adaptativa.

3. **Suffern, K. G.** (1990). Quadtree algorithms for contouring functions of two variables. *The Computer Journal*, 33(5), 402–407. — Refinamento adaptativo via Quadtree para contorno de funções implícitas.

4. **Fuller, R. B.** (1954). *Building construction* (U.S. Patent 2,682,235). — Patente do domo geodésico, aplicação prática da subdivisão icosaédrica.

5. **Figueiredo, L. H.** (2026). Notas de aula — Modelagem Geométrica, IMPA. — Referência principal do curso, incluindo classificação de caixas, enumeração espacial e representação por fronteira.
